...
async function fetchWinRateAndRank(uid) {
  try {
    ...
    const [wlRes, profileRes] = await Promise.all([
      fetch(`https://api.opendota.com/api/players/${steamId32}/wl`, { cache: 'no-store' }),
      fetch(`https://api.opendota.com/api/players/${steamId32}`, { cache: 'no-store' }),
    ]);
...
export default function FindClient() {
  ...
  // auth + текущая команда пользователя
  useEffect(() => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const usersData = await Promise.all(usersSnapshot.docs.map(async d => {
          const data = d.data();
          const { winRate, rank } = await fetchWinRateAndRank(data.steamId || '');
          return { id: d.id, ...data, rank, winRate };
        }));
        setGamers(usersData);
      } catch (error) {
  ...
  const handleJoinRequest = async (teamRow) => {
      ...
      <div className="max-w-6xl mx-auto p-6">
        ...
        {view === 'teams' && (
          <div className="space-y-6">
            {teams.map(teamRow => (
              <div key={teamRow.id} className="border p-4 rounded shadow bg-white">
                <div className="flex justify-between items-center">
                  <div>
                    <Link href={`/team/${teamRow.id}`} className="text-lg font-semibold text-blue-600 hover:underline">
                      {teamRow.name}
                    </Link>
                    <div className="text-sm text-gray-600">
                      {teamRow.members?.length || 0} {t('find.members')}
                    </div>
                  </div>
                  {teamRow.openForRequests && user && !user.teamId && (
                    <button
                      onClick={() => handleJoinRequest(teamRow)}
